...
public class ParallaxMovement : MonoBehaviour {
    ...
    void Start () {
        ...
        cameraTransform = GameObject.Find("Camera BG").transform;//Camera.main.transform;
    ...
    void Regenerate()
    {
        ...
        {
            case ScrollDirection.LeftToRight:
                transform.position = cameraTransform.GetComponent<Camera>().ViewportToWorldPoint(new Vector3(1f + limitOffScreen, Random.Range(0f, 1f), 10f));
                break;
            case ScrollDirection.RightToLeft:
                transform.position = cameraTransform.GetComponent<Camera>().ViewportToWorldPoint(new Vector3(-limitOffScreen, Random.Range(0f, 1f), 10f));
                break;
            case ScrollDirection.DownToUp:
                transform.position = cameraTransform.GetComponent<Camera>().ViewportToWorldPoint(new Vector3(Random.Range(0f, 1f), 1f + limitOffScreen, 10f));
                break;
            case ScrollDirection.UpToDown:
                transform.position = cameraTransform.GetComponent<Camera>().ViewportToWorldPoint(new Vector3(Random.Range(0f, 1f), -limitOffScreen, 10f));
                break;
        }
	...
	void Update () {
        ...
        {
            case ScrollDirection.LeftToRight:
                if(cameraTransform.GetComponent<Camera>().WorldToViewportPoint(transform.position).x < -limitOffScreen)
                {
                ...
                break;
            case ScrollDirection.RightToLeft:
                if (cameraTransform.GetComponent<Camera>().WorldToViewportPoint(transform.position).x > 1f + limitOffScreen)
                {
                    switch (behaviourOnExit)
                    {
                        case BehaviourOnExit.Destroy:
                ...
                break;
            case ScrollDirection.DownToUp:
                if (cameraTransform.GetComponent<Camera>().WorldToViewportPoint(transform.position).y < -limitOffScreen)
                {
                    switch (behaviourOnExit)
                    {
                        case BehaviourOnExit.Destroy:
                ...
                break;
            case ScrollDirection.UpToDown:
                if (cameraTransform.GetComponent<Camera>().WorldToViewportPoint(transform.position).y > 1f + limitOffScreen)
                {
                    switch (behaviourOnExit)
                    {
                        case BehaviourOnExit.Destroy:
